...
const RoadMap = ({ lat, lng }) => {
  ...
  const calculateOffset = (x1, y1, x2, y2, laneIndex, totalLanes) => {
    const laneWidth = window.innerWidth * 0.0017;
    ...
    >
      <Layer scaleY={-1} y={window.innerWidth * 0.4}>
        {Object.entries(roads).map((node) => (
          <Circle
            key={node[1].id}
            x={node[1].lng * window.innerWidth * 0.6}
            y={node[1].lat * window.innerWidth * 0.4}
            radius={window.innerWidth * 0.000825}
            fill="black"
          />
        ))}
            ...
            for (let i = 0; i < lanes; i++) {
              ...
              lines.push(
                <Line
                  ...
                  stroke="black"
                  strokeWidth={window.innerWidth * 0.00165}
                />
              );
            }
